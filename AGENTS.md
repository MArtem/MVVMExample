...
the agent to load it. If the canonical checkout is unavailable, require the tracked
...
- **Смена модели:** `не требуется`, `рекомендуется`, or `требуется`; include the target route for the latter two
- **Фаза:** current phase
...
2. Read its current Level 0 set once, including current task plan/handoff when present.
3. Load only task-relevant routes.
4. For MVVMExample product work, read
   `/Users/Artem/.zenflow/worktrees/documentation-vault/apps/MVVMExample/MANIFEST.md`.

The canonical reusable source is
`/Users/Artem/.zenflow/worktrees/documentation-vault/reusable/`. If a selected local route file is
missing or known stale, load the same path from `reusable/baseline/` and report the drift. Do not
read the full manifest, archived task history, legacy-reference tree, or all docs by default.
...
## Model Routing
- Apply the current `./docs/MODEL_ROUTING_RULE.md` before meaningful planning, implementation,
  review, or package adoption. Do not use retired GPT-5.4/5.5 routing text.
- The user-selected operating mode persists until explicitly changed. Proceed on an adequate
  current route; otherwise stop before task actions and require the documented switch.
- Use targeted inspection, one bounded patch, one relevant static check, then user-owned runtime
  verification. Stop when continued work no longer materially improves product-code quality.
...
- The project-local static gate may evolve only for demonstrated, high-signal project risks; do not enter a gate-hardening loop without product-quality benefit.
...
- The current authoritative project-only overlay is
