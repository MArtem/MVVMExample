...
readonly PROJECT="TCACase.xcodeproj"
readonly SCHEME="TCACase"
readonly UNIT_TEST_PLAN="TCACase"
readonly UI_TEST_PLAN="TCACaseUI"
readonly BUILD_DESTINATION="generic/platform=iOS Simulator"
readonly TEST_DESTINATION="${TCA_CASE_TEST_DESTINATION:-platform=iOS Simulator,name=Any iOS Simulator Device}"
readonly CASE_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
readonly DERIVED_DATA_PATH="${TCA_CASE_DERIVED_DATA_PATH:-${CASE_ROOT}/.xcode-derived-data/TCACase}"
readonly CLONED_PACKAGES_PATH="${TCA_CASE_XCODE_PACKAGE_CACHE:-${CASE_ROOT}/.xcode-package-cache/TCACase}"
readonly RESULT_BUNDLE_ROOT="${TCA_CASE_RESULT_BUNDLE_PATH:-${CASE_ROOT}/.xcode-result-bundles/TCACase}"
...
Levels:
  ...
  build      Build TCACase with a generic iOS Simulator destination
