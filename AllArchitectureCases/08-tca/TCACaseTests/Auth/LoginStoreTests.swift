...
struct LoginStoreTests {
    ...
    func demoCredentialsPopulateFormOnlyWhenConfigured() {
        let hiddenStore = LoginStore(repository: ControllableAuthRepository(), demoCredentials: nil, onLoginSuccess: { _ in })
    ...
    func loginSuccessTrimsUsernameKeepsPasswordAndPublishesSession() async {
        ...
        let store = LoginStore(repository: repository, onLoginSuccess: { receivedSession = $0 })
        store.username = "  ada  "
    ...
    func loginFailureMapsToUserSafeErrorAndDoesNotPublishSession() async {
        let repository = ControllableAuthRepository()
        var successCount = 0
        let store = LoginStore(repository: repository, onLoginSuccess: { _ in successCount += 1 })
        store.username = "ada"
