...
enum APIConfigurationError: LocalizedError, Equatable, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .missingProductionBaseURL:
            return "TCA_CASE_API_BASE_URL is required when demo API fallback is disabled."
...
struct APIConfiguration: Sendable {
    ...
    ///
    /// Environment keys:
    /// - `TCA_CASE_API_BASE_URL`
    /// - `TCA_CASE_API_TIMEOUT_SECONDS`
    /// - `TCA_CASE_ALLOW_DEMO_CREDENTIALS`
    static func current(
    ...
    static func make(environment: [String: String]) throws -> APIConfiguration {
        ...
        let allowsDemoCredentials = environment["TCA_CASE_ALLOW_DEMO_CREDENTIALS"]
        ...
        let timeout = environment["TCA_CASE_API_TIMEOUT_SECONDS"]
    ...
    static func makeBaseURL(
        ...
        guard let configuredBaseURL = environment["TCA_CASE_API_BASE_URL"] else {
