...
struct LoginPresenterTests {
    ...
    func demoCredentialsPopulateFormOnlyWhenConfigured() {
        let hiddenStore = LoginPresenter(repository: ControllableAuthRepository(), demoCredentials: nil, onLoginSuccess: { _ in })
    ...
    func loginSuccessTrimsUsernameKeepsPasswordAndPublishesSession() async {
        ...
        let store = LoginPresenter(repository: repository, onLoginSuccess: { receivedSession = $0 })
        store.username = "  ada  "
    ...
    func loginFailureMapsToUserSafeErrorAndDoesNotPublishSession() async {
        let repository = ControllableAuthRepository()
        var successCount = 0
        let store = LoginPresenter(repository: repository, onLoginSuccess: { _ in successCount += 1 })
        store.username = "ada"
