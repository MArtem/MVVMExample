...
final class LoginInteractor {
    ...
    init(
        ...
        self.worker = LoginWorker(repository: repository)
    ...
    private func login() {
        ...
        loginTask = Task { [worker, onLoginSuccess] in
            do {
                let session = try await worker.login(
                    username: username,
                    password: password
                )
...
/// Clean Swift Worker for authentication I/O.
///
/// The worker is the only login-scene role that talks to the repository; the interactor owns business flow and state transitions.
@MainActor
struct LoginWorker {
