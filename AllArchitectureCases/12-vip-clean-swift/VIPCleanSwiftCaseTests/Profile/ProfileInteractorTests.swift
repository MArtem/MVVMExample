...
@MainActor
@Suite("Profile Interactor tests")
struct ProfileInteractorTests {
    ...
    func appearedLoadsCurrentProfileIntoContentState() async {
        ...
        let interactor = makeProfileInteractor(repository: repository)
    ...
    func profileUpdateAppliesReturnedProfileWithoutNetworkReload() async {
        ...
        interactor.profileUpdated(makeUserProfile(firstName: "Grace", lastName: "Hopper"))
    ...
    func editAndLogoutIntentsDelegateToRouterAndSessionOwner() async {
        ...
        interactor.editTapped()
        #expect(router.path.isEmpty == true)

        interactor.profileUpdated(makeUserProfile(firstName: "Ada", lastName: "Lovelace"))
        interactor.editTapped()
...
@MainActor
private func makeProfileInteractor(repository: ControllableProfileLoadRepository) -> ProfileInteractor {
    ProfileInteractor(
        session: makeAuthSession(),
        repository: repository,
