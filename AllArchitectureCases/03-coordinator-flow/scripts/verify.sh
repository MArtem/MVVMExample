...
readonly PROJECT="CoordinatorFlowCase.xcodeproj"
readonly SCHEME="CoordinatorFlowCase"
readonly UNIT_TEST_PLAN="CoordinatorFlowCase"
readonly UI_TEST_PLAN="CoordinatorFlowCaseUI"
readonly BUILD_DESTINATION="generic/platform=iOS Simulator"
readonly TEST_DESTINATION="${COORDINATOR_FLOW_CASE_TEST_DESTINATION:-platform=iOS Simulator,name=Any iOS Simulator Device}"
readonly CASE_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
readonly DERIVED_DATA_PATH="${COORDINATOR_FLOW_CASE_DERIVED_DATA_PATH:-${CASE_ROOT}/.xcode-derived-data/CoordinatorFlowCase}"
readonly CLONED_PACKAGES_PATH="${COORDINATOR_FLOW_CASE_XCODE_PACKAGE_CACHE:-${CASE_ROOT}/.xcode-package-cache/CoordinatorFlowCase}"
readonly RESULT_BUNDLE_ROOT="${COORDINATOR_FLOW_CASE_RESULT_BUNDLE_PATH:-${CASE_ROOT}/.xcode-result-bundles/CoordinatorFlowCase}"
...
Levels:
  ...
  build      Build CoordinatorFlowCase with a generic iOS Simulator destination
