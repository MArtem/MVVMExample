...
final class NewsListReactor {
    ...
    private func mutateAsync(_ action: Action) async {
        switch action {
        case .refreshRequested:
    ...
    private func runAsync(_ effect: SideEffect) async {
        switch effect {
        case .refreshFirstPage:
            await refresh()
        default:
