...
@MainActor
@Suite("Profile Store tests")
struct ProfileReactorTests {
    ...
    func appearedLoadsCurrentProfileIntoContentState() async {
        ...
        let store = makeProfileReactor(repository: repository)
    ...
    func profileUpdateAppliesReturnedProfileWithoutNetworkReload() async {
        ...
        store.profileUpdated(makeUserProfile(firstName: "Grace", lastName: "Hopper"))
    ...
    func editAndLogoutIntentsDelegateToRouterAndSessionOwner() async {
        ...
        store.profileUpdated(makeUserProfile(firstName: "Ada", lastName: "Lovelace"))
        store.editTapped()
        store.logoutTapped()
...
@MainActor
private func makeProfileReactor(repository: ControllableProfileLoadRepository) -> ProfileReactor {
    ProfileReactor(
        session: makeAuthSession(),
        repository: repository,
