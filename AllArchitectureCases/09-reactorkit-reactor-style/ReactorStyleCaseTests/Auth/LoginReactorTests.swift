...
struct LoginReactorTests {
    ...
    func demoCredentialsPopulateFormOnlyWhenConfigured() {
        let hiddenStore = LoginReactor(repository: ControllableAuthRepository(), demoCredentials: nil, onLoginSuccess: { _ in })
    ...
    func loginSuccessTrimsUsernameKeepsPasswordAndPublishesSession() async {
        ...
        let store = LoginReactor(repository: repository, onLoginSuccess: { receivedSession = $0 })
        store.username = "  ada  "
    ...
    func loginFailureMapsToUserSafeErrorAndDoesNotPublishSession() async {
        let repository = ControllableAuthRepository()
        var successCount = 0
        let store = LoginReactor(repository: repository, onLoginSuccess: { _ in successCount += 1 })
        store.username = "ada"
