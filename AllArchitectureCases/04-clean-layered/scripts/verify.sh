...
readonly PROJECT="CleanLayeredCase.xcodeproj"
readonly SCHEME="CleanLayeredCase"
readonly UNIT_TEST_PLAN="CleanLayeredCase"
readonly UI_TEST_PLAN="CleanLayeredCaseUI"
readonly BUILD_DESTINATION="generic/platform=iOS Simulator"
readonly TEST_DESTINATION="${CLEAN_LAYERED_CASE_TEST_DESTINATION:-platform=iOS Simulator,name=Any iOS Simulator Device}"
readonly CASE_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
readonly DERIVED_DATA_PATH="${CLEAN_LAYERED_CASE_DERIVED_DATA_PATH:-${CASE_ROOT}/.xcode-derived-data/CleanLayeredCase}"
readonly CLONED_PACKAGES_PATH="${CLEAN_LAYERED_CASE_XCODE_PACKAGE_CACHE:-${CASE_ROOT}/.xcode-package-cache/CleanLayeredCase}"
readonly RESULT_BUNDLE_ROOT="${CLEAN_LAYERED_CASE_RESULT_BUNDLE_PATH:-${CASE_ROOT}/.xcode-result-bundles/CleanLayeredCase}"
...
Levels:
  ...
  build      Build CleanLayeredCase with a generic iOS Simulator destination
