...
readonly PROJECT="ModularFeatureSlicedCase.xcodeproj"
readonly SCHEME="ModularFeatureSlicedCase"
readonly UNIT_TEST_PLAN="ModularFeatureSlicedCase"
readonly UI_TEST_PLAN="ModularFeatureSlicedCaseUI"
readonly BUILD_DESTINATION="generic/platform=iOS Simulator"
readonly TEST_DESTINATION="${MODULAR_FEATURE_SLICED_CASE_TEST_DESTINATION:-platform=iOS Simulator,name=Any iOS Simulator Device}"
readonly CASE_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
readonly DERIVED_DATA_PATH="${MODULAR_FEATURE_SLICED_CASE_DERIVED_DATA_PATH:-${CASE_ROOT}/.xcode-derived-data/ModularFeatureSlicedCase}"
readonly CLONED_PACKAGES_PATH="${MODULAR_FEATURE_SLICED_CASE_XCODE_PACKAGE_CACHE:-${CASE_ROOT}/.xcode-package-cache/ModularFeatureSlicedCase}"
readonly RESULT_BUNDLE_ROOT="${MODULAR_FEATURE_SLICED_CASE_RESULT_BUNDLE_PATH:-${CASE_ROOT}/.xcode-result-bundles/ModularFeatureSlicedCase}"
...
Levels:
  ...
  build      Build ModularFeatureSlicedCase with a generic iOS Simulator destination
