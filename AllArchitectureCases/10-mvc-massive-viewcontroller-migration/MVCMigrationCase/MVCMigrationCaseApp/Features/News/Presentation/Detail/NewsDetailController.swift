...
/// MVC migration controller for article detail.
///
/// Ownership:
/// Created by the detail screen for one route payload.
///
/// Migration boundary:
/// This type intentionally owns the detail screen state, loading task, favorite task, optimistic update, persistence merge, and repository calls. That mirrors an incremental legacy controller extraction while keeping one controller per screen instead of a single app-wide massive object.
@MainActor
@Observable
