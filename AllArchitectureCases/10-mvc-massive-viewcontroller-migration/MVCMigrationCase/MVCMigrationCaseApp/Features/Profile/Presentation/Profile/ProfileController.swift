...
/// MVC migration controller for the profile screen.
///
/// Ownership:
/// Created by the profile screen composition layer for the authenticated session.
///
/// Migration boundary:
/// This type intentionally owns loading, routing callbacks, presentation state, and logout forwarding for one legacy-shaped screen controller. It remains screen-scoped so the migration case does not collapse the full app into one massive object.
@MainActor
@Observable
