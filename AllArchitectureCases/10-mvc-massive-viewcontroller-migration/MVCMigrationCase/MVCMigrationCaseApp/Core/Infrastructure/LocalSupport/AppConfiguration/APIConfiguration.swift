...
enum APIConfigurationError: LocalizedError, Equatable, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .missingProductionBaseURL:
            return "MVC_MIGRATION_CASE_API_BASE_URL is required when demo API fallback is disabled."
...
struct APIConfiguration: Sendable {
    ...
    ///
    /// Environment keys:
    /// - `MVC_MIGRATION_CASE_API_BASE_URL`
    /// - `MVC_MIGRATION_CASE_API_TIMEOUT_SECONDS`
    /// - `MVC_MIGRATION_CASE_ALLOW_DEMO_CREDENTIALS`
    static func current(
    ...
    static func make(environment: [String: String]) throws -> APIConfiguration {
        ...
        let allowsDemoCredentials = environment["MVC_MIGRATION_CASE_ALLOW_DEMO_CREDENTIALS"]
        ...
        let timeout = environment["MVC_MIGRATION_CASE_API_TIMEOUT_SECONDS"]
    ...
    static func makeBaseURL(
        ...
        guard let configuredBaseURL = environment["MVC_MIGRATION_CASE_API_BASE_URL"] else {
