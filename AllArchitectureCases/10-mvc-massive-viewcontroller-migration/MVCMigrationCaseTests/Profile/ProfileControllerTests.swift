...
struct ProfileControllerTests {
    ...
    func appearedLoadsCurrentProfileIntoContentState() async {
        ...
        let store = makeProfileController(repository: repository)
...
@MainActor
private func makeProfileController(repository: ControllableProfileLoadRepository) -> ProfileController {
    ProfileController(
        session: makeAuthSession(),
        repository: repository,
