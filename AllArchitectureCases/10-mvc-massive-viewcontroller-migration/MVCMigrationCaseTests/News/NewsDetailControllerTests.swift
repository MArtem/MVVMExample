...
@MainActor
@Suite("News detail Store tests")
struct NewsDetailControllerTests {
    ...
    func appearedLoadsArticleDetailIntoContentState() async {
        ...
        let store = makeDetailStore(repository: repository)

        store.appeared()
        let id = await repository.waitForDetailRequest(at: 0)

        #expect(id == 7)
        guard case .loading(let placeholder) = store.state else {
    ...
    func detailLoadOverlaysSharedInteractionState() async {
        ...
        let repository = ControllableNewsDetailRepository()
        let store = makeDetailStore(repository: repository, interactionStore: interactionStore)
        ...
        assertDetailContent(store.state, isFavorite: true, likesText: "Likes 99")
    }
    ...
    func cancelledDetailLoadDoesNotPublishStaleResponse() async {
        ...
        assertDetailContent(store.state, isFavorite: false, likesText: "Likes 10")
    }
    ...
    func favoriteSuccessKeepsLocalOptimisticCountAfterStaleServerAcknowledgement() async throws {
        ...
        let repository = ControllableNewsDetailRepository()
        let store = makeDetailStore(repository: repository, interactionStore: interactionStore)
        await loadDetailContent(store: store, repository: repository, article: makeDetailArticle(isLiked: false, likesCount: 10))

        store.favoriteTapped()
        let call = await repository.waitForToggleRequest(at: 0)

        #expect(call.articleID == 7)
        #expect(call.isLiked == true)
        assertDetailContent(store.state, isFavorite: true, likesText: "Likes 11")
...
@MainActor
private func makeDetailStore(repository: ControllableNewsDetailRepository) -> NewsDetailController {
    makeDetailStore(repository: repository, interactionStore: ArticleInteractionStore())
}
