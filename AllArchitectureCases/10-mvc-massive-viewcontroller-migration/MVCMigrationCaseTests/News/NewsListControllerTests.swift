...
@MainActor
@Suite("News list Store tests")
struct NewsListControllerTests {
    ...
    func refreshFailurePreservesVisibleContentAndShowsBanner() async {
        let repository = ControllableNewsRepository()
        let store = makeStore(repository: repository)
        await loadInitialContent(store: store, repository: repository, articles: makeArticles(count: 2))

        let refreshTask = Task { await store.refreshRequested() }
        let request = await repository.waitForRefreshRequest(at: 0)

        #expect(request == NewsPageRequest(limit: 30, skip: 0))
        guard case .refreshing(let refreshingContent) = store.state else {
    ...
    func refreshAppliesPersistedOptimisticInteractionState() async {
        ...
        let repository = ControllableNewsRepository()
        let store = makeStore(repository: repository, interactionStore: interactionStore)
        await loadInitialContent(store: store, repository: repository, articles: makeArticles(count: 2))
        ...
        assertCard(store.state, id: 1, likeState: .liked, likesText: "99")
        assertCard(store.state, id: 2, likeState: .notLiked, likesText: "2")
    }
    ...
    func unlikeSuccessUsesLocalOptimisticCountInsteadOfStaleServerCount() async {
        let repository = ControllableNewsRepository()
        let store = makeStore(repository: repository)
        await loadInitialContent(store: store, repository: repository, articles: [
        ...
        assertCard(store.state, id: 1, likeState: .notLiked, likesText: "4")
    ...
    func paginationRequestIsBackpressuredWhilePageIsAlreadyLoading() async {
        let repository = ControllableNewsRepository()
        let store = makeStore(repository: repository)
        await loadInitialContent(store: store, repository: repository, articles: makeArticles(count: 30))

        store.loadNextPageIfNeeded(currentItemID: 28)
        let request = await repository.waitForLoadRequest(at: 1)
        store.loadNextPageIfNeeded(currentItemID: 29)
        await drainMainActorTasks()
    ...
    func paginationPageMergeAppliesPersistedInteractionState() async {
        ...
        assertCard(store.state, id: 31, likeState: .liked, likesText: "99")
        assertCard(store.state, id: 32, likeState: .notLiked, likesText: "32")
    }
    ...
    func paginationFailureCanBeRetriedWithoutAdvancingSkip() async {
        ...
        guard case .content(let failedContent) = store.state else {
        ...
        assertCard(store.state, id: 31, likeState: .notLiked, likesText: "31")
    }
}
...
@MainActor
private func makeStore(repository: ControllableNewsRepository) -> NewsListController {
    makeStore(repository: repository, interactionStore: ArticleInteractionStore())
}
