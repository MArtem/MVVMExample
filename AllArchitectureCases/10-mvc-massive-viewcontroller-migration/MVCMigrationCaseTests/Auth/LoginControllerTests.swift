...
struct LoginControllerTests {
    @Test("Demo credentials populate form only when configured")
    func demoCredentialsPopulateFormOnlyWhenConfigured() {
        let hiddenStore = LoginController(repository: ControllableAuthRepository(), demoCredentials: nil, onLoginSuccess: { _ in })
        hiddenStore.useDemoCredentialsTapped()
        #expect(hiddenStore.username == "")
        #expect(hiddenStore.password == "")
        #expect(hiddenStore.state.showsDemoCredentialsButton == false)

        let shownStore = LoginController(
            repository: ControllableAuthRepository(),
            demoCredentials: DemoCredentials(username: "demo-user", password: "demo-pass"),
            onLoginSuccess: { _ in }
        )
        shownStore.useDemoCredentialsTapped()
        #expect(shownStore.username == "demo-user")
        #expect(shownStore.password == "demo-pass")
        #expect(shownStore.state.showsDemoCredentialsButton == true)
    }
    ...
    func loginSuccessTrimsUsernameKeepsPasswordAndPublishesSession() async {
        ...
        let store = LoginController(repository: repository, onLoginSuccess: { receivedSession = $0 })
        store.username = "  ada  "
    ...
    func loginFailureMapsToUserSafeErrorAndDoesNotPublishSession() async {
        let repository = ControllableAuthRepository()
        var successCount = 0
        let store = LoginController(repository: repository, onLoginSuccess: { _ in successCount += 1 })
        store.username = "ada"
        ...
        #expect(store.state.isLoading == false)
        #expect(store.state.errorMessage == "Your session expired. Please sign in again.")
