...
enum APIConfigurationError: LocalizedError, Equatable, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .missingProductionBaseURL:
            return "REDUX_ELM_UDF_CASE_API_BASE_URL is required when demo API fallback is disabled."
...
struct APIConfiguration: Sendable {
    ...
    ///
    /// Environment keys:
    /// - `REDUX_ELM_UDF_CASE_API_BASE_URL`
    /// - `REDUX_ELM_UDF_CASE_API_TIMEOUT_SECONDS`
    /// - `REDUX_ELM_UDF_CASE_ALLOW_DEMO_CREDENTIALS`
    static func current(
    ...
    static func make(environment: [String: String]) throws -> APIConfiguration {
        ...
        let allowsDemoCredentials = environment["REDUX_ELM_UDF_CASE_ALLOW_DEMO_CREDENTIALS"]
        ...
        let timeout = environment["REDUX_ELM_UDF_CASE_API_TIMEOUT_SECONDS"]
    ...
    static func makeBaseURL(
        ...
        guard let configuredBaseURL = environment["REDUX_ELM_UDF_CASE_API_BASE_URL"] else {
