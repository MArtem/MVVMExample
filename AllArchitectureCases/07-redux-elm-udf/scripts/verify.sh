...
readonly PROJECT="ReduxElmUDFCase.xcodeproj"
readonly SCHEME="ReduxElmUDFCase"
readonly UNIT_TEST_PLAN="ReduxElmUDFCase"
readonly UI_TEST_PLAN="ReduxElmUDFCaseUI"
readonly BUILD_DESTINATION="generic/platform=iOS Simulator"
readonly TEST_DESTINATION="${REDUX_ELM_UDF_CASE_TEST_DESTINATION:-platform=iOS Simulator,name=Any iOS Simulator Device}"
readonly CASE_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
readonly DERIVED_DATA_PATH="${REDUX_ELM_UDF_CASE_DERIVED_DATA_PATH:-${CASE_ROOT}/.xcode-derived-data/ReduxElmUDFCase}"
readonly CLONED_PACKAGES_PATH="${REDUX_ELM_UDF_CASE_XCODE_PACKAGE_CACHE:-${CASE_ROOT}/.xcode-package-cache/ReduxElmUDFCase}"
readonly RESULT_BUNDLE_ROOT="${REDUX_ELM_UDF_CASE_RESULT_BUNDLE_PATH:-${CASE_ROOT}/.xcode-result-bundles/ReduxElmUDFCase}"
...
Levels:
  ...
  build      Build ReduxElmUDFCase with a generic iOS Simulator destination
