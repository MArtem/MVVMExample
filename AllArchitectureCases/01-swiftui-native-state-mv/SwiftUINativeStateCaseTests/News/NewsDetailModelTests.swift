...
@MainActor
@Suite("News detail Model tests")
struct NewsDetailModelTests {
    ...
    func appearedLoadsArticleDetailIntoContentState() async {
        ...
        let model = makeDetailModel(repository: repository)

        model.appeared()
        let id = await repository.waitForDetailRequest(at: 0)

        #expect(id == 7)
        guard case .loading(let placeholder) = model.state else {
    ...
    func detailLoadOverlaysSharedInteractionState() async {
        ...
        let repository = ControllableNewsDetailRepository()
        let model = makeDetailModel(repository: repository, interactionStore: interactionStore)
        ...
        assertDetailContent(model.state, isFavorite: true, likesText: "Likes 99")
    }
    ...
    func cancelledDetailLoadDoesNotPublishStaleResponse() async {
        ...
        assertDetailContent(model.state, isFavorite: false, likesText: "Likes 10")
    }
    ...
    func favoriteSuccessKeepsLocalOptimisticCountAfterStaleServerAcknowledgement() async throws {
        ...
        let repository = ControllableNewsDetailRepository()
        let model = makeDetailModel(repository: repository, interactionStore: interactionStore)
        await loadDetailContent(model: model, repository: repository, article: makeDetailArticle(isLiked: false, likesCount: 10))
        ...
        assertDetailContent(model.state, isFavorite: true, likesText: "Likes 11")
...
@MainActor
private func makeDetailModel(repository: ControllableNewsDetailRepository) -> NewsDetailModel {
    makeDetailModel(repository: repository, interactionStore: ArticleInteractionStore())
}
