...
@MainActor
@Suite("News list Model tests")
struct NewsListModelTests {
    ...
    func refreshFailurePreservesVisibleContentAndShowsBanner() async {
        let repository = ControllableNewsRepository()
        let model = makeModel(repository: repository)
        await loadInitialContent(model: model, repository: repository, articles: makeArticles(count: 2))

        let refreshTask = Task { await model.refreshRequested() }
        let request = await repository.waitForRefreshRequest(at: 0)

        #expect(request == NewsPageRequest(limit: 30, skip: 0))
        guard case .refreshing(let refreshingContent) = model.state else {
    ...
    func refreshAppliesPersistedOptimisticInteractionState() async {
        ...
        let repository = ControllableNewsRepository()
        let model = makeModel(repository: repository, interactionStore: interactionStore)
        await loadInitialContent(model: model, repository: repository, articles: makeArticles(count: 2))
        ...
        assertCard(model.state, id: 1, likeState: .liked, likesText: "99")
        assertCard(model.state, id: 2, likeState: .notLiked, likesText: "2")
    }
    ...
    func unlikeSuccessUsesLocalOptimisticCountInsteadOfStaleServerCount() async {
        let repository = ControllableNewsRepository()
        let model = makeModel(repository: repository)
        await loadInitialContent(model: model, repository: repository, articles: [
        ...
        assertCard(model.state, id: 1, likeState: .notLiked, likesText: "4")
    ...
    func paginationRequestIsBackpressuredWhilePageIsAlreadyLoading() async {
        let repository = ControllableNewsRepository()
        let model = makeModel(repository: repository)
        await loadInitialContent(model: model, repository: repository, articles: makeArticles(count: 30))

        model.loadNextPageIfNeeded(currentItemID: 28)
        let request = await repository.waitForLoadRequest(at: 1)
        model.loadNextPageIfNeeded(currentItemID: 29)
        await drainMainActorTasks()
    ...
    func paginationPageMergeAppliesPersistedInteractionState() async {
        ...
        assertCard(model.state, id: 31, likeState: .liked, likesText: "99")
        assertCard(model.state, id: 32, likeState: .notLiked, likesText: "32")
    }
    ...
    func paginationFailureCanBeRetriedWithoutAdvancingSkip() async {
        ...
        guard case .content(let failedContent) = model.state else {
        ...
        assertCard(model.state, id: 31, likeState: .notLiked, likesText: "31")
    }
}
...
@MainActor
private func makeModel(repository: ControllableNewsRepository) -> NewsListModel {
    makeModel(repository: repository, interactionStore: ArticleInteractionStore())
}
