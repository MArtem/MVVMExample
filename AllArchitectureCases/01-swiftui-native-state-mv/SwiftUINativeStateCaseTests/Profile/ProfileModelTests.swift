...
struct ProfileModelTests {
    ...
    func appearedLoadsCurrentProfileIntoContentState() async {
        ...
        let model = makeProfileModel(repository: repository)
    ...
    func profileUpdateAppliesReturnedProfileWithoutNetworkReload() async {
        ...
        model.profileUpdated(makeUserProfile(firstName: "Grace", lastName: "Hopper"))
    ...
    func editAndLogoutIntentsDelegateToRouterAndSessionOwner() async {
        ...
        model.profileUpdated(makeUserProfile(firstName: "Ada", lastName: "Lovelace"))
        model.editTapped()
        model.logoutTapped()
...
@MainActor
private func makeProfileModel(repository: ControllableProfileLoadRepository) -> ProfileModel {
    ProfileModel(
        session: makeAuthSession(),
        repository: repository,
