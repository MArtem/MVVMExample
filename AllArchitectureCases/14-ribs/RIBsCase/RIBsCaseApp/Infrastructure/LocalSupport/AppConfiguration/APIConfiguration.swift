...
enum APIConfigurationError: LocalizedError, Equatable, Sendable {
    ...
    var errorDescription: String? {
        switch self {
        case .missingProductionBaseURL:
            return "RIBSCASE_API_BASE_URL is required when demo API fallback is disabled."
...
struct APIConfiguration: Sendable {
    ...
    ///
    /// Environment keys:
    /// - `RIBSCASE_API_BASE_URL`
    /// - `RIBSCASE_API_TIMEOUT_SECONDS`
    /// - `RIBSCASE_ALLOW_DEMO_CREDENTIALS`
    static func current(
    ...
    static func make(environment: [String: String]) throws -> APIConfiguration {
        ...
        let allowsDemoCredentials = environment["RIBSCASE_ALLOW_DEMO_CREDENTIALS"]
        ...
        let timeout = environment["RIBSCASE_API_TIMEOUT_SECONDS"]
    ...
    static func makeBaseURL(
        ...
        guard let configuredBaseURL = environment["RIBSCASE_API_BASE_URL"] else {
