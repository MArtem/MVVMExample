...
@MainActor
@Suite("News list Interactor tests")
struct NewsListInteractorTests {
    ...
    func appearedLoadsFirstPageIntoContentState() async {
        ...
        guard case .content(let content) = interactor.state else {
    ...
    func refreshFailurePreservesVisibleContentAndShowsBanner() async {
        let repository = ControllableNewsRepository()
        let interactor = makeInteractor(repository: repository)
        await loadInitialContent(interactor: interactor, repository: repository, articles: makeArticles(count: 2))

        let refreshTask = Task { await interactor.refreshRequested() }
        let request = await repository.waitForRefreshRequest(at: 0)

        #expect(request == NewsPageRequest(limit: 30, skip: 0))
        guard case .refreshing(let refreshingContent) = interactor.state else {
    ...
    func refreshAppliesPersistedOptimisticInteractionState() async {
        ...
        let repository = ControllableNewsRepository()
        let interactor = makeInteractor(repository: repository, interactionStore: interactionStore)
        await loadInitialContent(interactor: interactor, repository: repository, articles: makeArticles(count: 2))
        ...
        assertCard(interactor.state, id: 1, likeState: .liked, likesText: "99")
        assertCard(interactor.state, id: 2, likeState: .notLiked, likesText: "2")
    }
    ...
    func unlikeSuccessUsesLocalOptimisticCountInsteadOfStaleServerCount() async {
        let repository = ControllableNewsRepository()
        let interactor = makeInteractor(repository: repository)
        await loadInitialContent(interactor: interactor, repository: repository, articles: [
        ...
        assertCard(interactor.state, id: 1, likeState: .notLiked, likesText: "4")
    ...
    func paginationRequestIsBackpressuredWhilePageIsAlreadyLoading() async {
        let repository = ControllableNewsRepository()
        let interactor = makeInteractor(repository: repository)
        await loadInitialContent(interactor: interactor, repository: repository, articles: makeArticles(count: 30))

        interactor.loadNextPageIfNeeded(currentItemID: 28)
        let request = await repository.waitForLoadRequest(at: 1)
        interactor.loadNextPageIfNeeded(currentItemID: 29)
        await drainMainActorTasks()
    ...
    func paginationPageMergeAppliesPersistedInteractionState() async {
        ...
        assertCard(interactor.state, id: 31, likeState: .liked, likesText: "99")
        assertCard(interactor.state, id: 32, likeState: .notLiked, likesText: "32")
    }
    ...
    func paginationFailureCanBeRetriedWithoutAdvancingSkip() async {
        ...
        guard case .content(let failedContent) = interactor.state else {
        ...
        interactor.retryLoadNextPageTapped()
        ...
        assertCard(interactor.state, id: 31, likeState: .notLiked, likesText: "31")
    }
}
...
@MainActor
private func makeInteractor(repository: ControllableNewsRepository) -> NewsListInteractor {
    makeInteractor(repository: repository, interactionStore: ArticleInteractionStore())
}
