...
@MainActor
@Suite("News detail Interactor tests")
struct NewsDetailInteractorTests {
    ...
    func appearedLoadsArticleDetailIntoContentState() async {
        ...
        let interactor = makeDetailInteractor(repository: repository)

        interactor.appeared()
        let id = await repository.waitForDetailRequest(at: 0)

        #expect(id == 7)
        guard case .loading(let placeholder) = interactor.state else {
    ...
    func loadFailureMapsToFullScreenUserSafeError() async {
        ...
        guard case .error(let message) = interactor.state else {
    ...
    func detailLoadOverlaysSharedInteractionState() async {
        ...
        let repository = ControllableNewsDetailRepository()
        let interactor = makeDetailInteractor(repository: repository, interactionStore: interactionStore)
        ...
        assertDetailContent(interactor.state, isFavorite: true, likesText: "Likes 99")
    }
    ...
    func cancelledDetailLoadDoesNotPublishStaleResponse() async {
        ...
        guard case .loading = interactor.state else {
        ...
        assertDetailContent(interactor.state, isFavorite: false, likesText: "Likes 10")
    }
    ...
    func favoriteSuccessKeepsLocalOptimisticCountAfterStaleServerAcknowledgement() async throws {
        ...
        let repository = ControllableNewsDetailRepository()
        let interactor = makeDetailInteractor(repository: repository, interactionStore: interactionStore)
        await loadDetailContent(interactor: interactor, repository: repository, article: makeDetailArticle(isLiked: false, likesCount: 10))

        interactor.favoriteTapped()
        let call = await repository.waitForToggleRequest(at: 0)

        #expect(call.articleID == 7)
        #expect(call.isLiked == true)
        assertDetailContent(interactor.state, isFavorite: true, likesText: "Likes 11")
...
@MainActor
private func makeDetailInteractor(repository: ControllableNewsDetailRepository) -> NewsDetailInteractor {
    makeDetailInteractor(repository: repository, interactionStore: ArticleInteractionStore())
}

@MainActor
private func makeDetailInteractor(
