...
@MainActor
@Suite("Profile edit Interactor tests")
struct ProfileEditInteractorTests {
    ...
    func saveTrimsRequestPublishesUpdatedProfileAndPopsRoute() async {
        ...
        interactor.firstName = "  Ada  "
        interactor.lastName = "  Lovelace\n"
        interactor.email = "  ada@example.com  "

        interactor.saveTapped()
        let call = await repository.waitForUpdateCall()
        await repository.waitForUpdateContinuation()

        #expect(interactor.state.isSaving == true)
        ...
        #expect(interactor.state.isSaving == false)
    ...
    func saveFailureUsesUserSafeErrorMappingAndKeepsRouteVisible() async {
        ...
        #expect(interactor.state.isSaving == false)
        #expect(interactor.state.errorMessage == AppErrorMapper.userMessage(for: AppAPIError.offline))
    ...
    func profileEditSaveSuccessUpdatesProfilePresentation() async {
        ...
        let payload = makePayload()
        router.openEdit(payload)
        let editInteractor = ProfileEditInteractor(
            payload: payload,
            repository: repository,
            router: router,
            onSaveSuccess: { profileInteractor.profileUpdated($0) }
        )

        editInteractor.firstName = "  Katherine  "
        editInteractor.lastName = "  Johnson  "
        editInteractor.email = "  katherine@example.com  "

        editInteractor.saveTapped()
        let call = await repository.waitForUpdateCall()
        await repository.waitForUpdateContinuation()
        ...
        guard case .content(let content) = profileInteractor.state else {
