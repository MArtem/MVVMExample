...
@MainActor
@Suite("Profile edit Presenter tests")
struct ProfileEditPresenterTests {
    ...
    func saveTrimsRequestPublishesUpdatedProfileAndPopsRoute() async {
        ...
        presenter.firstName = "  Ada  "
        presenter.lastName = "  Lovelace\n"
        presenter.email = "  ada@example.com  "

        presenter.saveTapped()
        let call = await repository.waitForUpdateCall()
        await repository.waitForUpdateContinuation()

        #expect(presenter.state.isSaving == true)
        ...
        #expect(presenter.state.isSaving == false)
    ...
    func saveFailureUsesUserSafeErrorMappingAndKeepsRouteVisible() async {
        ...
        #expect(presenter.state.isSaving == false)
        #expect(presenter.state.errorMessage == AppErrorMapper.userMessage(for: AppAPIError.offline))
    ...
    func profileEditSaveSuccessUpdatesProfilePresentation() async {
        ...
        editPresenter.firstName = "  Katherine  "
        editPresenter.lastName = "  Johnson  "
        editPresenter.email = "  katherine@example.com  "

        editPresenter.saveTapped()
        let call = await repository.waitForUpdateCall()
        await repository.waitForUpdateContinuation()
        ...
        guard case .content(let content) = profilePresenter.state else {
