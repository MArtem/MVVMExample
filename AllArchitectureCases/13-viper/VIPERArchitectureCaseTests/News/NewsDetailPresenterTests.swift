...
@MainActor
@Suite("News detail Presenter tests")
struct NewsDetailPresenterTests {
    ...
    func appearedLoadsArticleDetailIntoContentState() async {
        ...
        let presenter = makeDetailPresenter(repository: repository)

        presenter.appeared()
        let id = await repository.waitForDetailRequest(at: 0)

        #expect(id == 7)
        guard case .loading(let placeholder) = presenter.state else {
    ...
    func loadFailureMapsToFullScreenUserSafeError() async {
        ...
        guard case .error(let message) = presenter.state else {
    ...
    func detailLoadOverlaysSharedInteractionState() async {
        ...
        let repository = ControllableNewsDetailRepository()
        let presenter = makeDetailPresenter(repository: repository, interactionStore: interactionStore)
        ...
        assertDetailContent(presenter.state, isFavorite: true, likesText: "Likes 99")
    }
    ...
    func cancelledDetailLoadDoesNotPublishStaleResponse() async {
        ...
        guard case .loading = presenter.state else {
        ...
        assertDetailContent(presenter.state, isFavorite: false, likesText: "Likes 10")
    }
    ...
    func favoriteSuccessKeepsLocalOptimisticCountAfterStaleServerAcknowledgement() async throws {
        ...
        let repository = ControllableNewsDetailRepository()
        let presenter = makeDetailPresenter(repository: repository, interactionStore: interactionStore)
        await loadDetailContent(presenter: presenter, repository: repository, article: makeDetailArticle(isLiked: false, likesCount: 10))
        ...
        assertDetailContent(presenter.state, isFavorite: true, likesText: "Likes 11")
...
@MainActor
private func makeDetailPresenter(repository: ControllableNewsDetailRepository) -> NewsDetailPresenter {
    makeDetailPresenter(repository: repository, interactionStore: ArticleInteractionStore())
}

@MainActor
private func makeDetailPresenter(
