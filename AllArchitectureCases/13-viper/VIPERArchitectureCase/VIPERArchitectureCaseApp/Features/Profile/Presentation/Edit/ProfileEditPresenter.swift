...
final class ProfileEditPresenter {
    ...
    ///
    /// Concurrency:
    /// A new save request cancels any in-flight save task owned by this Presenter.
    func saveTapped() {
        save()
    }
    ...
    private func save() {
        ...
        saveTask = Task { [interactor, payload] in
            do {
                let updatedProfile = try await interactor.updateProfile(id: payload.id, request: request)
...
/// VIPER Interactor for the profile-edit module.
///
/// Owns profile mutation work and keeps persistence/network details out of the presenter.
@MainActor
struct ProfileEditInteractor {
