...
final class NewsListPresenter {
    ...
    private func load() {
        ...
        loadTask = Task { [interactor, viewStateBuilder, pageSize] in
            do {
    ...
    private func loadNextPage() {
        ...
        loadNextPageTask = Task { [interactor, viewStateBuilder] in
            defer { loadNextPageTask = nil }
            do {
                let nextPageArticles = try await interactor.loadPage(request)
                try Task.checkCancellation()
    ...
    private func synchronizeVisibleContentWithInteractionStore() {
        ...
        let mergedArticles = interactor.mergeInteractionState(into: articles)
    ...
    private func toggleLike(articleID: Int) {
        ...
        likeTasks[articleID] = Task { [interactor, viewStateBuilder] in
            defer { likeTasks[articleID] = nil }
            do {
                try await interactor.persistLike(articleID: articleID, isLiked: targetIsLiked)
                try Task.checkCancellation()
                interactor.acknowledgeLikeSuccess(optimisticArticle, articleID: articleID)
                articles = articles.map { $0.id == articleID ? optimisticArticle : $0 }
